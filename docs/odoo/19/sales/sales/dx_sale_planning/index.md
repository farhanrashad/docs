...
Technical module: `dx_sale_planning`
...
- Dependencies: `sale_management`, `sales_team`, `mail`

Install the declared dependencies first, then install `Sales Planning` from Odoo Apps. Do not assume a dependency is available in another Odoo version.
...
3. Install `Sales Planning` and follow the source-backed menus and workflows below.
...
- Static source files: 8 Python, 8 XML, and 1 CSV.
- Detected model names: `sale.plan`, `sale.plan.line`, `sale.plan.revision`
